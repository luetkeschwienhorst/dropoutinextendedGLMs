...
i <- 2
for (j in 1:3) {
    print(paste("Model: model_",sep="",i,j))
    model <- paste("model_",sep="",i,j)
    assign(model,simulation_def(data_distribution=data_distr,phi=phi,nu=1,N=NULL,
                                b=b_normal,Db=Db_normal,Db_inv=Db_inv_normal,D2b=D2b_normal,X=X,
    ...
    image <- paste("model_",sep="",i,1,"_to_",i,j,"_normal.RData")
